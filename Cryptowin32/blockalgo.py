...
from Transcrypt.Cryptowin32 import Counter
from Transcrypt.Cryptowin32.strxor import strxor
from Transcrypt.Cryptowin32.number import long_to_bytes, bytes_to_long
import Transcrypt.Cryptowin32.Counter
from Transcrypt.Cryptowin32 import CMAC
from Transcrypt.Cryptowin32.CMAC import _SmoothMAC
from Transcrypt.Cryptowin32.KDF import _S2V
from Transcrypt.Cryptowin32 import galois
...
class BlockAlgo:
    ...
    def __init__(self, factory, key, *args, **kwargs):
        ...
        if self.mode == MODE_CCM:
            ...
            if self._mac_len not in (4, 6, 8, 10, 12, 14, 16):
                raise ValueError(
                    "Parameter 'mac_len' must be even and in the range 4..16")
    ...
    def _start_gcm(self, factory, key, *args, **kwargs):
        ...
        if not (self._mac_len and 4 <= self._mac_len <= 16):
            raise ValueError(
                "Parameter 'mac_len' must not be larger than 16 bytes")
    ...
    def _start_siv(self, factory, key, *args, **kwargs):
        ...
        if rem:
            raise ValueError(
                "MODE_SIV requires a key twice as long as for the underlying cipher")
    ...
    def _start_eax(self, factory, key, *args, **kwargs):
        ...
        counter_obj = Transcrypt.Cryptowin32.Counter.new(
            self.block_size * 8,
    ...
    def encrypt(self, plaintext):
        if self.mode == MODE_OPENPGP:
            padding_length = (self.block_size - len(plaintext) %
            ...
            if padding_length > 0:
                ...
                if self._done_last_block:
                    raise ValueError(
                        "Only the last chunk is allowed to have length not multiple of %d bytes",
                        self.block_size)
    ...
    def decrypt(self, ciphertext):
        ...
        if self.mode == MODE_OPENPGP:
            padding_length = (self.block_size - len(ciphertext) %
                              self.block_size) % self.block_size
            if padding_length > 0:
                # CFB mode requires ciphertext to have length multiple
                # of block size,
